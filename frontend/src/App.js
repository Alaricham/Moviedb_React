...
class App extends Component {
  ...
  componentDidMount() {
    ...
    axios.get(`http://localhost:3000/main`).then(results => {
  ...
  getAll = id => {
    this.setState({ready: false})
    axios.get(`http://localhost:3000/movie?id=${id}`).then(results => {
      let data = results.data;
  ...
  getSearch = (type, name) => {
    this.setState({ready: false})
    axios.get(`http://localhost:3000/search?search=${name}`).then(results => {
      let data = results.data;
